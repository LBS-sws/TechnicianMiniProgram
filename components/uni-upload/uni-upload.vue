<template>
	<view class="s-add-list">
		<view
			...
			:key="index"
		>
			<image :src="item[filedImage]" :url="item.url" @click="showImgs(index)" class="s-add-list-img" :mode="imgMode"></image>
	...
	methods: {
		...
		**/
		...
		uploadBase(index) {
			...
			// 验证后端
			if (this.url == '') {
				uni.showToast({ title: '没有配置api', icon: 'none' });
				return;
			}
			// 检查是否是默认值
			...
			const upTask = uni.uploadFile({
				...
				success: res => {
					...
					// if (uploadRes.code != 0) {
					// 	uni.showToast({ title: '上传失败 : ' + uploadRes.msg, icon: 'none' });
					// 	this.error(index);
		...
		setItems(items) {
          ...
          if(items[i] !== 'undefined' || items[i] !== undefined){
            this.imageList.push({ url: `${this.$baseUrl_imgs}` + items[i],result: items[i], progress: 100 });
          }
				}
			}
		}
